import { parseClientMessage, serialize, stale, subscribe } from "rxfy-protocol";
...
describe("createWsClient", () => {
  it("send delivers when open and drops when not (no buffering — onOpen replay owns recovery)", () => {
    ...
    expect(sockets[0]!.sent.map((m) => parseClientMessage(m))).toEqual([frame]);
    transport.close();
  });
  ...
  it("onOpen fires on first open and every reconnect", () => {
    ...
    vi.advanceTimersByTime(10); // reconnect creates a fresh socket
    sockets[1]!.ws.open();
    expect(opens).toBe(2);
