...
export const app = new Hono()
  ...
  .get("/posts", (c) => {
    ...
    // `$grant`; the client lifts it and subscribes on its own WebSocket. Stateless — no session.
  ...
  .post("/posts/:id/comments", zValidator("json", CreateCommentInputSchema), (c) => {
    ...
    // Every tab holding a grant for this post's detail channel gets a live "1 new comment" badge.
