 ...
 * browser gets its own client from api-client.ts. The render's minted `session` rides along as the
 * live session header, so everything served is registered for live pushes under it. `.api` unwraps the app's `/api` basePath.
 */
