...
// its own store (store.ts) and only drives the sync server's grant half (`sync.serve` / `sync.renew`
// / `sync.hydration`) plus `sync.touch` for stale badges — so `memoryStorage()` carries no
