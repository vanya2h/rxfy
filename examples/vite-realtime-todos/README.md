...
other tab right away, with no list refetch. The server pushes that entity only to the
...
- **Targeted live updates.** The client opens one WebSocket and tells the server which entity
  ...
  loaded, with no per-query wiring (`src/live/useStoreSubscriptions.ts`). The server keeps **one
...
- Live cross-tab list membership would need a separate list-level message; the per-entity
  socket here covers single entities by design.
