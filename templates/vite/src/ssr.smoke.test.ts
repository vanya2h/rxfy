...
describe("SSR", () => {
  it("renders the todos page with data resolved and a hydration payload", async () => {
    ...
    const { html, state } = await render("/", sync, api.request);
    // Seeded todo is in the first-paint HTML — no PENDING flash.
    expect(html).toContain("Open this app in a second tab");
    expect(html).not.toContain("Loading…");
    // Hydration payload + the signed channel grants ride along in <!--app-state-->; the client lifts
    // the grants (readSsrGrants) and subscribes on its own socket.
