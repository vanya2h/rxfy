...
Add model relations with per-state joins. Declare relation fields in a model schema with `ref()`/`refArray()` and join them per fetch with `single(Model).with({ rel: true })` (Prisma-`include` style — a nested map `{ rel: { sub: true } }` joins recursively), so list and detail payloads share one normalized store. New `useModelStoreValue(model, id)` gives a non-throwing reactive read for components that may render whether or not a relation was joined.
