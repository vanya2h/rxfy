...
```ts
// The config is a discriminated union: remote (fetchFn + params) or local (initial).
function useStateData<TParams, TShape, TMutations>(
  config:
    ...
    | {
        state: StateDescriptor<TParams, TShape, TMutations>;
        initial: TShape; // pure local/sync state — no fetch (see below)
      },
): StateHandle<TShape, TMutations>
```

The two forms are mutually exclusive: pass `fetchFn` + `params` to fetch, or `initial` for
[local/sync state](#local--sync-state--initial-instead-of-fetchfn). `data$` emits the normalized
query shape — entity ids for `array`/`single` fields, and [plain value fields](/core-concepts/state#plain-value-fields)
(bare zod schemas in the `model`) passed through with their real value.
...
## Local / sync state — `initial` instead of `fetchFn`

When there is no server fetch, pass **`initial`** (the denormalized shape) instead of
`fetchFn` / `params`. The state becomes pure local/sync: `data$` is seeded to `FULFILLED`
**synchronously** from `initial` — entity fields normalize into their stores, plain fields pass
through — so there is **no `PENDING` flash** and nothing is ever fetched. `set`, `setRaw`, and
`mutations` work exactly as in remote mode.

```tsx
import { useStateData, Pending } from "rxfy-react";
import { counterState } from "./counter"; // defineState({ params: z.object({}), model: { count: z.number(), todos: array(TodoModel) }, mutations: { inc: (p) => ({ ...p, count: p.count + 1 }) } })

function Counter() {
  const { data$, mutations, reload } = useStateData({
    state: counterState,
    initial: { count: 0, todos: [] },
  });
  ...
  return (
    <Pending value$={data$}>
      {({ count }) => (
        <>
          <span>{count}</span>
          <button onClick={() => mutations.inc()}>+1</button>
...
In local mode, **`reload()` resets** the state back to `initial` (discarding local edits) — there
is nothing to re-fetch. A `key` still shares the atom across components via the registry, so a keyed
local state acts as a seeded-once shared slice. `initial` is captured at first render; later changes
to it are ignored — use `reload()` to reset.
