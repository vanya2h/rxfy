...
| Live app — the framework packages (`rxfy-server`, `rxfy-ws`, `rxfy-client`) on top | `rxfy-framework` |
...
- Grants and live hydration: `live.serve` (signs a channel grant as `$grant`), `live.hydration`, `createLiveClient` with grant renewal, `readSsrGrants`, and state channels
