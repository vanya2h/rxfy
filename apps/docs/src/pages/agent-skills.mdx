...
- Grants and sync hydration: `sync.serve` (signs a channel grant as `$grant`), `sync.hydration`, `createSyncClient` with grant renewal, `readSsrGrants`, and state channels
