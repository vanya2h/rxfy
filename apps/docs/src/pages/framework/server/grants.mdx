...
live client strips `$grant`, normalizes the rest, derives the entity topics the payload normalized
into, and sends one `subscribe` frame carrying `{ grant, entities }`. The channel subscription is
authorized by the token; the entity topics are accepted alongside any currently-valid grant. SSR
grants take the same path via [`readSsrGrants`](/react/live-client).
...
`createServer` requires a `secret` — the HMAC key it signs grants with. The WebSocket server
...
:::danger[Entity ids MUST be unguessable in live-enabled apps]
Entity patches carry **full rows** and fan out on raw `name:id` topics, gated only by the presence
of a currently-valid grant — the server cannot statelessly verify that an entity belongs to the
grant's state. With guessable ids (serial integer primary keys) any valid grant would let a client
watch arbitrary rows. **Use unguessable ids — `crypto.randomUUID()`, not auto-increment PKs.**

The mandate covers exactly the models whose rows are published: resource-backed models written
through `live.create` / `live.update` in a live-enabled app. (A model written via `live.*`
publishes patches even if no state serving it ever calls `live.serve`, so its ids must be opaque
regardless.) Store-only apps, client-only models, and non-live states impose nothing on ids.
:::
...
See [`createServer`](/framework/server/create-server) for `live.serve` / `live.renew` /
