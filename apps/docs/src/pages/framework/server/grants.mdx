...
time — it _signs_. `live.serve` returns the parsed payload plus a reserved `$grant` field: a JWT
...
A read endpoint wraps its result in `live.serve` — it parses the raw payload (the state's _input_
