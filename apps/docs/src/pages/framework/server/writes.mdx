...
- `live` — the `Live` object from [`createLive`](/framework/server/create-server).
...
await live.create(postResource, { id: newId(), userId, title, body }, { touch: [touch(postsState, {})] });
...
export const live = createLive({ storage: drizzleStorage(db), hub });
```
