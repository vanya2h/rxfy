...
`LiveStorage` adapter, and each resource carries a **binding** the adapter knows how to persist. You
pick the adapter and hand it to [`createLive`](/framework/server/create-server); everything else — the
hub, grants, `live.*` writes — stays the same. Two official adapters ship today, with more data
providers on the way.

## rxfy-server-drizzle

The Drizzle / Postgres adapter binds Drizzle tables to rxfy models and persists the live server's
writes.
...
A `LiveStorage` that runs `create` / `update` / `delete` against the bound tables. Hand it to
`createLive`:
...
export const live = createLive({ storage: drizzleStorage(db), hub: createInMemoryHub(), secret: SECRET });
```

`drizzleStorage(db)` takes any Drizzle `PgDatabase`; each write returns the affected row, which the
live server publishes as a `patch`.
...
source. Reach for it in demos, tests, and prototypes — it's a drop-in `LiveStorage` you swap for the
...
A `LiveStorage` over those collections. It's stateless — the data lives in each collection's binding —
...
export const live = createLive({ storage: memoryStorage(), hub: createInMemoryHub(), secret: SECRET });

await live.create(postCollection, { id: "2", title: "New", body: "…" }); // stores + publishes a patch
```
