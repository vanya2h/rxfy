...
function UserRow({ id }: { id: string }) {
  ...
  return <Pending value$={user$}>{(user) => <li>{user.name}</li>}</Pending>;
}
...
2. Each `UserRow` resolved its own entity with `store.get(id)`, an observable of that one
   user. `Pending` rendered a fallback until it settled.
...
`UserRow` unwraps its one entity. rxfy keeps values wrapped until the leaf that renders
them — unwrap late, and both loading UI and re-renders stay exactly where the data is
used.
