...
:::note[Is this rung for you?]
The Sync Client fits best when you're starting a new app, or when your backend already runs
on Node with [Drizzle](https://orm.drizzle.team) — `rxfy-server` binds synced resources
directly to Drizzle tables. With a different backend stack, stay on the
[Create Store](/getting-started/create-store) rung and fetch your API your way; you can
...
becomes the sync topic namespace, and its `getKey` locates the primary key. The registry
...
- [Sync messages](/framework/server/messages): `patch` vs `stale`, and what travels on the wire.
...
- [Sync Client](/react/live-client): `createLiveClient`, reconnection, and stale handling in React.

Then build the whole thing end to end: the [Sync blog guide](/guides/sync-blog) walks
through the runnable [`vite-blog-framework`](/examples) example — schema, server, sockets,
SSR grants, and both sync behaviors in two tabs.
