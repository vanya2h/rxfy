...
SSR also mints the **session** a live client uses to receive pushes. See
...
Live apps use a different entry point for the same idea: [`live.hydration(registry)`](/framework/server/sessions)
mints a session, registers everything the render served, and returns the hydration script in one
call — the client adopts the session with [`getSessionId`](/react/live-client#getsessionid)
instead of calling `dehydrate` / `hydrationScript` directly.
