...
[Model](/core-concepts/model), [State](/core-concepts/state), and
...
![teamState and profileState both normalize their entities by id into a single shared UserModel store (the source of truth); the Team page reads u1 and u2 from it and the Profile page reads u1, each via useModelStore.](/core-concepts/normalization.png)
