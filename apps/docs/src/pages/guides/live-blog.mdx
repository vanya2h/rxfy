...
// reads — an inline async arrow as fetchFn
const fetchPosts = async () => (await api.posts.$get()).json();
const fetchPostDetail = async ({ postId }: { postId: string }) => {
