...
On the client, create the WebSocket transport and live client, adopt the SSR-minted session, then
pass the live client to `StoreProvider`:
...
- `createLiveClient` — sends one `hello(session)` frame and wires incoming `patch` / `stale`
  events to the shared registry. There is nothing to subscribe: the server already knows what this
  session was served.
- `readSsrSession()` — reads the session id `live.hydration` embedded in the page; falls back to a
  freshly minted id on a CSR-only load.
...
- [Live client](/react/live-client) — `createLiveClient`, `readSsrSession`, and stale handling in React.
