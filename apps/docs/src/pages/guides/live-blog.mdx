...
`render` takes `live` as a parameter rather than importing it — in dev, Vite's SSR module graph is
separate from the server's, and the hub (which holds subscription state) must stay a single
instance across both. See [Sessions](/framework/server/sessions) for the full model.
...
- The session defaults to `getSessionId()` (from `rxfy-client`), which adopts the session id
  ...
  WebSocket instead. The API client attaches the same id to every request with `sessionHeaders`:
