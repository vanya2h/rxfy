...
Normalization already did the hard part. Because every entity lives in **one** keyed cell
(see [Normalization](/normalization)), a server push doesn't need to know who's looking at
what — it carries the new entity value, drops it into the shared store, and every component
subscribed to that entity re-renders. No refetch, no re-selection.

It also hands you the **subscription set for free**: the ids a client fetched and normalized
are *exactly* the entities it should receive updates for. So instead of broadcasting every
change to every client, each connection subscribes to the ids it holds, and the server pushes
a change only to the connections that hold that id.
...
[`registry.added$`](/models-state#observing-what-enters-the-store) stream — and ingests pushes with
[`store.setMany`](/models-state#modelstore-entity). Your existing `useStateData` /
`useModelStore` components don't change at all — nothing in them is live-aware.
...
A topic is just a string at runtime, but giving it a **branded template literal type** —
shared by both sides, like the schema — means only a value built by the `topic()` constructor
...
validation on the server — so they can't drift. The Drizzle table mirrors it (or use
`drizzle-zod` to derive one from the other).

```ts
// shared/todo.ts — imported by client and server
import { z } from "zod";
...
import { topic, type Topic } from "../shared/topic";
...
import { hub } from "./hub";
import type { Topic } from "../shared/topic";
...
fetched it* — so the store's contents are exactly the topics the client wants live. That makes
the manager trivial: it accumulates topics and tells the server about the ones it hasn't heard
...
> **The trade-off.** This subscribes to *everything the client has loaded* and never drops a
...
export function createLiveClient(socket: WebSocket) {
  const desired = new Set<Topic>(); // every topic we've ever wanted (grows, never shrinks)
  let active = new Set<Topic>();    // topics the server currently knows

  const send = (topics: Topic[]) => {
    if (topics.length && socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify({ type: "add", topics }));
    }
  };

  // Tell the server only about topics it hasn't been told yet (desired − active), then record
  // that it now knows them. So a burst of `want` calls collapses to one `add` of just the new ones.
  const reconcile = () => {
    send([...desired].filter((t) => !active.has(t)));
    active = new Set(desired);
  };

  // Reconnect: the server forgot us, so `active` is empty again — replay the whole desired set.
  ...
  return {
    want(topic: Topic) {
      if (desired.has(topic)) return; // already subscribed (or queued) — nothing to send
      desired.add(topic);
      reconcile();
    },
  };
...
`active` is the client's memory of what the server knows; `desired` is everything it wants.
`reconcile` sends only the gap between them, so `want` is idempotent and cheap to call once per
entity. No `remove`, no slices, no refcounting — the store's lifecycle *is* the subscription's.

### Drive subscriptions from the store

Here's the payoff, and the reason for the core [`registry.added$`](/models-state#observing-what-enters-the-store)
stream. It emits `{ name, key }` the first time any **named** entity lands in the store — from
the initial `useStateData` fetch, from SSR hydration, or from a live push — and **replays what's
already there** to a late subscriber. One subscription, mounted once, keeps the connection live
on exactly the store's contents. No component passes ids anywhere; there's no `useLiveQuery`.
...
export function useStoreSubscriptions() {
  ...
  useEffect(() => {
    const sub = registry.added$.subscribe(({ name, key }) => client.want(topic(name, key)));
...
const LiveContext = createContext<LiveClient | null>(null);
...
export function LiveProvider({ children }: { children: React.ReactNode }) {
  ...
  const client = useMemo(() => createLiveClient(socket), [socket]);
...
A component just fetches as it always would — **nothing in it is live-aware**. The moment
...
function TodoApp() {
  ...
  const { data$ } = useStateData(todosState, fetchTodos, params);
...
[`useModelStore`](/react#usemodelstore). When a push writes a row into its cell, only the rows
holding that id re-render — and only the clients that fetched it received the push at all.

## How it flows

```text
   Client A toggles todo u1               Client C never fetched u1
        |                                        (not subscribed)
        | POST /todos/u1/toggle                       |
        v                                             |
   Hono route ── writes row ──> Postgres (Drizzle)    |
        |                                             |
        | publish("todo", "u1", updated)              |
        v                                             |
   +-------------------------------------------------+ |
   |  hub: for each connection, deps.has("todo:u1")?  | |
   |  A {todo:u1, …} yes   B {todo:u1} yes   C {…} no  | |
   +-------------------------------------------------+ |
        |                    |                         x  (skipped)
        v                    v
   Client A             Client B
   store.setMany        store.setMany
        |                    |
        v                    v
   the one TodoModel cell for u1 — every subscriber of u1 re-renders,
   no refetch, no re-select; clients without u1 are untouched
```

## Adding and removing items

A push updates an entity's **value** — a rename, a toggle — and that reaches every subscriber
cleanly. It does **not** change which ids a query lists: `data$` is an id array owned by the
query cache (see [`useStateData`](/react#usestatedata)), and `setMany` only writes entity
cells. (A deleted row leaves the list but its topic stays subscribed — harmless, since the
server simply has nothing more to push for it.)
...
- **Same client** — a [`useStateData` mutation](/models-state#definestate) updates the local
  id list immediately (and the entity cells under it).
- **Other clients** — call the handle's `reload()` to re-pull the list, or push a list-level
  message that replaces the query's ids. The per-entity socket above is the sweet spot; list
  membership is a deliberately separate concern.
...
- **On reconnect**, `createLiveClient` replays the full desired set (every topic loaded this
