...
type DepsMessage = { type: "add" | "remove"; topics: string[] }; // e.g. ["todo:u1", "todo:u2"]
...
import { addClient, removeClient, addDeps, removeDeps, publish } from "./hub";
...
app.post("/todos/:id/toggle", async (c) => {
  ...
  publish("todo", updated.id, updated); // <- targeted live update
...
app.get(
  ...
  upgradeWebSocket(() => ({
    ...
    onMessage: (evt, ws) => {
      ...
      if (msg.type === "remove") removeDeps(ws, msg.topics);
    },
