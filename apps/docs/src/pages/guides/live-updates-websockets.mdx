...
(see [Normalization](/normalization)), a server push doesn't need to know which client renders
...
[`registry.added$`](/models-state#observing-what-enters-the-store) stream) and ingests pushes
with [`store.setMany`](/models-state#modelstore-entity). Your existing `useStateData` /
...
This is where the core [`registry.added$`](/models-state#observing-what-enters-the-store)
