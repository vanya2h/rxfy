...
A [model](/core-concepts/model#createmodel) is an entity type plus how to read its id. Each todo
...
export const TodoModel = createModel(z.object({ id: z.string(), title: z.string(), done: z.boolean() }), {
  getKey: (todo) => todo.id,
  name: "todo",
});
...
A [state](/core-concepts/state#definestate) declares the params it takes, the model fields it
...
## Step 6: Edit a title with a form, save on submit
...
Hold that draft in a local [`Atom`](/core-concepts/atom) seeded from the todo, focus the
field you're editing with a [`Lens`](/core-concepts/lens), and bind it with
[`useAtom`](/react#useatom-and-two-way-binding). The form never reads or writes the store;
...
```tsx
// TodoItem.tsx: render <EditTitle> instead of the static title
...
```tsx
// App.tsx: pass onRename alongside onToggle
...
- [Model](/core-concepts/model), [State](/core-concepts/state), and [React Bindings](/react): the full API.
- [Atom](/core-concepts/atom) and [Lens](/core-concepts/lens): the draft-and-save form here, in depth.
- [Live updates over WebSockets](/guides/live-updates-websockets) and [Pagination](/guides/pagination): common next tasks.
