...
it receives the normalized **ids**, the same shape `data$` emits), and a `merge` (fold a page
into the **denormalized** list, the way `set` works). Page 0 is `merge(initial, firstPage)`, so
it returns the full shape and goes through `useStateData`'s cache / SSR path unchanged.
...
function Feed() {
  ...
  const { data$, loadMore, isLoading, hasMore } = useStatePagedData({
    ...
    merge: ({ prev, page }) => ({ items: [...prev.items, ...page.items] }),
