...
`name` is the model's stable string identity for SSR dehydration and live topics —
...
users.get("1").get(); // { id: "1", name: "Alice" } — synchronous
users.get("1").subscribe(console.log); // reactive: emits on every change
```
...
```ts
type ModelStore<T> = {
  get(key: EntityKey<T>): IAtom<T>; // writable handle (see below); throws if not loaded
...
lets a live-update layer track exactly what the client holds without each query wiring its
...
const user$ = users.get("1"); // IAtom<User>
