...
const todosState = defineState({
  key: "todos", // stable identity for the SSR query cache and the live invalidation channel
...
type StateDescriptor<TParams, TShape, TMutations> = {
  key: string; // SSR query-cache identity, also derives the live invalidation channel
...
For live apps, `window` names the params that slice _within_ a dataset — page, cursor, sort —
