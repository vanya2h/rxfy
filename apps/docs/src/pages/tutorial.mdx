...
> after the lesson — here, just build.

## Before you start

You need a React 18+ app with TypeScript (a fresh Vite `react-ts` project works). Install
the packages and their peers:
...
## Step 1 — Wrap the app in `StoreProvider`
...
A [model](/models-state#createmodel) is an entity type plus how to read its id. Each todo
...
## Step 3 — Define a state and a fake fetch

A [state](/models-state#definestate) declares the params it takes, the model fields it
returns, and the mutations that change it. Add two mutations now — one to add a todo, one
to toggle `done`:
...
update the store in place — no re-fetch.

`params` is wrapped in `useMemo` so its identity is stable; a fresh object every render
would restart the fetch.

## Step 5 — Render one todo
...
## Step 6 — Edit a title, see it everywhere

Last step: make the title editable, and prove the store is one shared copy. `store.entity(id)`
is a writable handle on a todo; a [`Lens`](/core-concepts/lens) focuses one field of it;
[`useAtom`](/react#useatom-and-two-way-binding) binds that field to an input. Writes flow
back into the store, so every component showing this todo updates as you type.
...
Drop `<EditTitle id={todo.id} />` into the `TodoItem` render and type in it. The text in
the list row changes in the same keystroke, because the row and the input read the same
entity cell — not two copies.
...
- [Normalization](/normalization) — why `data$` emits ids, and what it buys you.
- [Models & State](/models-state) and [React Bindings](/react) — the full API.
- [Optimistic mutations](/guides/optimistic-mutations) and [Two-way form binding](/guides/form-binding) — common next tasks.
