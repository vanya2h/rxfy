...
In code, each page subscribes to its own `data$` (which carries only ids) and renders the same
by-id component, which reads the entity from the shared store:
...
The team page resolves `"u1"` against the same store the profile page wrote to — so both render
the same user, not two snapshots that can disagree.
