...
`useStateData`, `Pending`, `useModelStore`) are covered in [Model](/core-concepts/model),
[State](/core-concepts/state), and [React Bindings](/react), and appear here only to make
the idea concrete.
...
same path; see [State](/core-concepts/state#definestate).
...
re-fetching. The more often the same data appears across your app, the more this buys you.

## In rxfy

- [`createModel`](/core-concepts/model#createmodel) defines an entity type and its id (`getKey`).
- [`defineState`](/core-concepts/state#definestate) declares which fields hold which model; the fetch
